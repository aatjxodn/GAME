...
var ServerName = payloadObject.accessToken.NAME
...
function battingTrue() {
	...
	$('.game2Form_left_con').css('filter', 'brightness(70%)');
	$('.game2Form_left_con').css('background-color', '#32CD32');
...
function battingFalse() {
	...
	$('.game2Form_left_con').css('filter', 'brightness(100%)');
	$('.game2Form_left_con').css('background-color', '#32CD32');
}
...
function connect() {
	var wsUri = "ws://localhost:8081/BackAPI/webGameChatServer";
...
function showMessage(message) {
    ...
    if (message.substring(0, 3) === ServerName) {
        chatArea.innerHTML += '<div class="sendMessage">' + message + "</div><br>";
    } else {
        chatArea.innerHTML += '<div class="receivedMessage">' + message + "</div><br>";
    }
    chatArea.scrollTop = chatArea.scrollHeight; // 스크롤을 아래로 이동
}

function sendMessage() {
	
    var messageInput = document.getElementById("messageInput");
    var message = ServerName + " : " + messageInput.value;
